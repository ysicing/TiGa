...
func WithCache() Opt {
	return func(o *options) error {
		o.cacheDir = common.GetDefaultCacheDir()
		return nil
	}
}
...
func Download(local, remote string, opts ...Opt) (*Result, error) {
	...
	if local == "" {
		...
		if _, err := os.Stat(localPath); err == nil {
			log.GetInstance().Debugf("file %q already exists, skipping downloading from %q (and skipping digest validation)", localPath, remote)
			res := &Result{
	...
	if err := os.WriteFile(shadURL, []byte(remote), common.FileMode0600); err != nil {
		return nil, err
	}
	...
	if shadDigest != "" && o.expectedDigest != "" {
		if err := os.WriteFile(shadDigest, []byte(o.expectedDigest.String()), common.FileMode0600); err != nil {
			return nil, err
		}
	}
	res := &Result{
...
func canonicalLocalPath(s string) (string, error) {
	...
	return filepath.Abs(zos.ExpandPath(s))
}
...
func validateLocalFileDigest(localPath string, expectedDigest digest.Digest) error {
	...
	log.GetInstance().Debugf("verifying digest of local file %q (%s)", localPath, expectedDigest)
...
func downloadHTTP(localPath, url string, description string, expectedDigest digest.Digest) error {
	...
	log.GetInstance().Debugf("downloading %q into %q", url, localPath)
