...
func ChinaTrace(ch chan Result, i int) {
	hops, err := Trace(net.ParseIP(ChinaIPS[i]))
	if err != nil {
		s := fmt.Sprintf("%v %-15s %v", names[i], ChinaIPS[i], err)
		ch <- Result{i, s}
		return
	}
	...
	for _, h := range hops {
		for _, n := range h.Nodes {
			...
			s := fmt.Sprintf("%v %-15s %-23s", names[i], ChinaIPS[i], as)
			ch <- Result{i, s}
			return
		}
	}
	s := fmt.Sprintf("%v %-15s %v", names[i], ChinaIPS[i], color.SRed("测试超时"))
	ch <- Result{i, s}
}
